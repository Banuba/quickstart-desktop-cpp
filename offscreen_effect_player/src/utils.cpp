...
namespace bnb
{
color_plane bnb::convert_bpc8_to_y_plane(const bpc8_image_t& image, uint32_t width, uint32_t height)
{
    std::vector<uint8_t> y_pixels(width * height);
    ...
    return color_plane_vector(std::move(y_pixels));
}
...
color_plane bnb::convert_bpc8_to_uv_plane(const bpc8_image_t& image, uint32_t width, uint32_t height, bool legacy_yuv)
{
    std::vector<uint8_t> uv_pixels((width / 2) * (height / 2) * 2);
    ...
    return color_plane_vector(std::move(uv_pixels));
}
} // bnb